...
const typeIcons = {
  ...
  delivery_confirmation: 'bg-green-500/20 text-green-400', processing_update: 'bg-yellow-500/20 text-yellow-400',
  cancellation: 'bg-red-500/20 text-red-400', promotion: 'bg-pink-500/20 text-pink-400',
    ...
    <div className="h-full flex flex-col gap-2 py-3">
      <div className="flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          <BackButton />
          <h1 className="text-lg font-bold text-white font-['Playfair_Display']">Notifications</h1>
        </div>
        <div className="flex items-center gap-2">
          {unreadCount > 0 && (
            <button onClick={markAllAsRead} className="px-3 py-1.5 bg-[#D4AF37]/20 text-[#D4AF37] rounded-lg text-xs font-medium hover:bg-[#D4AF37]/30 transition-colors">Mark All Read</button>
          )}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto space-y-1">
        {filtered.length === 0 && <div className="text-center py-8 text-[#6B7280] text-xs">{filter === 'Unread' ? 'No unread notifications' : 'No notifications'}</div>}
        {filtered.map((notification) => (
          <div key={notification.id} className={`bg-[#232326] rounded-lg border transition-colors ${notification.read ? 'border-[#2D2D30]/30 opacity-70' : 'border-[#D4AF37]/20'}`}>
            <div className="flex items-start gap-3 p-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <h4 className={`text-xs ${notification.read ? 'text-[#B8B8C2]' : 'text-white font-semibold'}`}>{notification.title}</h4>
                  {!notification.read && <span className="w-1.5 h-1.5 rounded-full bg-[#D4AF37] flex-shrink-0 mt-1" />}
                </div>
                <p className="text-xs text-[#6B7280]">{notification.message}</p>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] text-[#6B7280]">{formatDate(notification.createdAt)}</span>
                  {!notification.read && <button onClick={() => markAsRead(notification.id)} className="text-[10px] text-[#D4AF37] hover:underline">Read</button>}
                  <button onClick={() => handleDelete(notification.id)} className="text-[10px] text-[#6B7280] hover:text-red-400 ml-auto">Delete</button>
                </div>
              </div>
            </div>
          </div>
